...
int main(int argc, char *argv[]) {
  ...
  ULONG info = 2;
  HeapSetInformation(GetProcessHeap(), HeapCompatibilityInformation, &info,
                     sizeof(info));
#endif
#if 0 // defined(__SVR4)
{
  ...
  if (argc > 7) {
    ...
    // printf ("sleep = %ld, min = %d, max = %d, per thread = %d, num rounds =
    // %d, seed = %d, max_threads = %d, min_threads = %d\n",
    //    sleep_cnt, min_size, max_size, chperthread, num_rounds, seed,
...
void runloops(long sleep_cnt, int num_chunks) {
  ...
  printf(" %6.3f %.3f", (double)used_space / (1024 * 1024),
